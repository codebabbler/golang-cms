...
Every finding below is dispositioned against `docs/superpowers/specs/2026-07-11-architecture-review-remediation-design.md` (the owner-approved remediation design, Decision Log D-1…D-8, Sections 1–8). Section/subsection numbers cite that spec; all named document edits are committed in `docs/`.

| AR # | Sev | Finding | Disposition |
|---|---|---|---|
| AR-1 | Blocker | `access_rules` JSONB schema undefined; no vocabulary for end_user/api_key/anonymous principals. | Resolved by spec §1 (grant-matrix schema, evaluation algorithm, worked examples; new BR-RBAC-7; new `docs/architecture/12-access-rules.md`). |
| AR-2 | Blocker | Optimistic-locking contract self-contradicts for published records ("live row does not change" vs. "version increments every write"). | Resolved by spec §3.1 (BR-LIFE-7 CAS wording: system columns always advance, content columns freeze until publish). |
| AR-3 | Blocker | Concurrency claims wrong — `ALTER TABLE` rewrites take `ACCESS EXCLUSIVE`, blocking reads too; "reads proceed via MVCC" is false. | Resolved by spec §3.2 (03 EC-1/EC-12 correction; ops guidance; V2 `CREATE INDEX CONCURRENTLY` alternative). |
| AR-4 | Blocker | Proxy-trust rule ignores `X-Forwarded-For` entirely when Cloudflare fronts a public origin, mis-keying rate limits. | Resolved by spec §2.5 (new `CMS_TRUSTED_PROXY_CIDRS`; empty default preserves current heuristic; fails closed). |
| AR-5 | Blocker | No credential-recovery path for admins or end users; BR-RUNTIME-2 forbids email delivery; sole super admin lockout needs DB surgery. | Resolved by spec §2.1–2.3 (new BR-AUTH-12 recovery mode; admin-issued one-time resets; new BR-AUTH-13 end-user reset API). |
| AR-6 | High | No availability/uptime objective despite single-process, stop-replace-start deploys. | Resolved by spec §7.2 (new N-13 availability statement, ~99.5%-class, HA explicitly out of scope). |
| AR-7 | High | No RPO/RTO; `pg_dump`-only backup risks up to a full day of data loss. | Resolved by spec §7.1 (PITR required; new N-12: RPO ≤ 5 min, RTO ≤ 1 h; restore-drill runbook). |
| AR-8 | Medium | End-user account lifecycle unspecified — verification, password policy, deletion/export, `register`/`refresh` rate limits, enumeration posture. | Deferred to V2 (spec §2.6 resolves rate limits and password policy now — 10/30 per 15 min, 10/8-char minimums; email verification and account deletion/export remain deferred per §18 short/long-term). |
| AR-9 | High | Exactly-one-process assumption has no enforcement; a second replica would serve stale access rules silently. | Resolved by spec §4.1 (new BR-RUNTIME-8: process-lifetime advisory lock before listener open; new EC-16). |
| AR-10 | High | Anonymous public-read DoS — unindexed filters, unbounded `ILIKE`, per-page `COUNT(*)`, no statement timeout, no anonymous rate limit. | Resolved by spec §5.1 (new BR-API-4: public scope restricted to indexed/unique fields; also §5.2 opt-in counts, §4.4 `statement_timeout`). |
| AR-11 | Medium | No edge-cache contract (`Cache-Control`/`ETag`/purge-on-publish) for public GETs — the only 100× lever is undesigned. | Resolved by spec §5.4 (new BR-API-5: `public, s-maxage=60, stale-while-revalidate=60`; credentialed responses forced `no-store`; V2 purge-on-publish). |
| AR-12 | Medium | `/healthz` conflates liveness and readiness; a DB blip triggers restart loops or a full outage. | Resolved by spec §4.2 (new `/readyz` for DB readiness; `/healthz` stays pure liveness). |
| AR-13 | Medium | Ticker panic (e.g. in `jobs.Retention`) kills the whole process; no per-job isolation. | Resolved by spec §4.3 (tickers wrap each tick in panic recovery; failing tick logs and skips, never kills the process). |
| AR-14 | Medium | `cms_sessions`/`cms_refresh_tokens` grow unbounded — no cleanup job. | Resolved by spec §4.3 (`jobs.Retention` gains duties 4–6: purge expired sessions, used/expired reset tokens, stale refresh tokens). |
| AR-15 | Medium | In-memory rate-limit maps are unbounded-cardinality. | Resolved by spec §4.3 (rate-limiter maps bounded LRU with fixed entry cap). |
| AR-16 | High | JWT RS256 private key stored plaintext in DB; no rotation, `kid`, or JWKS. | Resolved by spec §2.4 (BR-AUTH-10 amended: AES-256-GCM at rest under `CMS_MASTER_SECRET`-derived key; `kid` claim; rotation runbook). |
| AR-17 | Medium | `CMS_SESSION_SECRET` has no defined function; "cookie signing" claim adds nothing to hashed-lookup sessions. | Documentation fix applied (spec §2.4 — variable renamed `CMS_MASTER_SECRET` with a defined role: at-rest encryption key derivation, not session signing). |
| AR-18 | Medium | 10,000-row offset cap applies to admin lists too, with no cursor escape valve. | Resolved by spec §5.5 (keyset cursors land in `query.Builder` in V1; exposed on admin list endpoints). |
| AR-19 | High | Media hardening thin — no MIME allowlist, `Content-Type` unbound in presign, no CORS spec, CSP contradicts media previews. | Resolved by spec §6 (compile-time MIME allowlist; `Content-Type` signed into presign + re-verified at finalize; bucket CORS policy; origin isolation; CSP `img-src`/`media-src` fix). |
| AR-20 | Low (by decision) | V1 audit trail only as durable as stdout logs; no queryable trail until V2. | Accepted trade-off, documented (spec §7.6 — named in REQUIREMENTS as F-16/BR-AUDIT-2 accepted limitation). |
| AR-21 | Low | Setup-token logging (BR-AUTH-11) contradicts hard rule 6 ("never log tokens"); no TTL stated. | Documentation fix applied (spec §8.3 — hard rule 6 amended with the setup/recovery-token exception + 30-minute TTL; this edit is committed in `CLAUDE.md` as of this same pass). |
| AR-22 | Medium (V2) | Webhooks (V2) have no durable delivery — in-process retry dies with the process. | Deferred to V2 (spec §8.11 — `cms_webhook_deliveries` outbox table specified now, built in V2). |
| AR-23 | Medium (V2) | Webhooks (V2) are user-configurable URLs with no SSRF defense. | Deferred to V2 (spec §8.11 — private/link-local/loopback deny + DNS re-resolution + no-redirect-to-private rules specified now, enforced in V2). |
| AR-24 | Medium | Per-email login limiter allows targeted admin lockout. | Accepted trade-off, documented (spec §2.6 — standard trade-off explicitly written into 05 rather than changed). |
| AR-25 | Low | Session fixation/rotation on login and CSRF-token rotation unstated. | Resolved by spec §2.6 (session ID and CSRF token rotate on login, stated as normative text in 05). |
| AR-26 | Medium | Media deletion lifecycle unspecified — who may delete, FK behavior, storage cleanup, revision interaction. | Resolved by spec §6 (destructive-gated deletion; 409 while referenced via FK RESTRICT; row-then-object delete order; orphan sweep as backstop). |
| AR-27 | Medium | DropCollection's effect on `cms_revisions` is unspecified, breaking the BR-SCHEMA-7 "revisions retain dropped data" promise. | Documentation fix applied (spec §8.1 — BR-SCHEMA-7 addendum: collection drop cascades fields, revisions, and table in one transaction; asymmetry with field-drop made explicit). |
| AR-28 | Low | Restore-from-trash status semantics implicit — a published record likely un-trashes straight back to public visibility. | Documentation fix applied (spec §8.2 — restore returns prior `status`; admin UI warns when restoring a published record). |
| AR-29 | Low | Admin-user deletion semantics unspecified — session cascade, `created_by` references, audit identity. | Resolved by spec §8.12 (deleting an admin cascades sessions/reset tokens; `created_by` persists by design; audit events store actor snapshots). |
| AR-30 | Medium | No GDPR erasure/export flow; append-only `cms_revisions` structurally conflicts with right-to-erasure. | Deferred to V2 (spec §7.3, new F-33 — erasure semantics specified now: hard delete + tombstoned `created_by` + revision-redaction capability; implementation in V2). |
| AR-31 | Low | Doc-generation artifacts ("the prompt's field-type reference", "generation contract") cite documents that don't exist in the repo. | Documentation fix applied (spec §8.13 — F-2 points to `07-data-model.md`; 11's "generation contract" phrase removed). |
| AR-32 | Low | `Publish` takes no expected version; publish racing unpublish/trash has last-write-wins semantics with no conflict signal. | Accepted trade-off, documented (not elevated in Decision Log D-1…D-8; the review's own §4 already characterizes the existing `revisionNo` parameter as making this "mostly benign" — no spec change made; remains an open low-priority nit). |
| AR-33 | Low | Clock skew unaddressed — `publish_at` comparisons should use DB time, not process clock. | Documentation fix applied (spec §8.5 — `publish_at` comparisons specified against database `now()`). |
| AR-34 | Low | No max-field-size validation on write; a single paste can create outsized revision snapshots. | Resolved by spec §8.6 (1 MiB per-field cap enforced in `Document.Set`). |
| AR-35 | Low | Startup catch-up publishes every missed scheduled record before the listener opens — unbounded startup time after a long outage. | Resolved by spec §8.4 (catch-up moved to the publisher's first tick immediately after the listener opens; BR-LIFE-9 wording adjusted). |
| AR-36 | Low | Drain contract's "zero dropped requests" promise is falsified by its own documented force-close path. | Documentation fix applied (spec §3.3 — N-6/BR-RUNTIME-6 reworded: zero dropped among requests completing within the 15 s window). |
| AR-37 | Low | Import-rule enforcement is review-only, not automated in CI. | Resolved by spec §8.9 (`depguard` enforces the import-rules table; added to `make test` prerequisites). |
| AR-38 | Medium | No `http.Server` timeouts, no per-request context deadline, no `statement_timeout` anywhere in the docs. | Resolved by spec §4.4 (new timeout table: ReadHeader 5 s / Read 30 s / Write 30 s / Idle 120 s; 25 s context deadline; `statement_timeout` 10 s/60 s; body caps 5 MiB/64 KiB). |
| AR-39 | Medium | Creates have no idempotency mechanism; a retried timed-out POST duplicates records. | Resolved by spec §4.5 (optional `Idempotency-Key` on public/API-key creates; new `cms_idempotency_keys` table; 24 h window). |
| AR-40 | Low | Sort-tiebreaker (`id`) needs composite `(field, id)` indexes; 07 specifies single-column B-trees only. | Resolved by spec §8.7 (indexed fields get composite `(field, id)` B-trees). |
| AR-41 | Low | Relation expansion (`?expand=`) doesn't specify batched loading vs. per-row N+1 lookups. | Resolved by spec §8.8 (`?expand=` resolves via a single `IN` query per relation field). |
| AR-42 | Medium | N-3/N-4 performance targets have no verification harness. | Resolved by spec §7.5 (new `make bench` target against a seeded 100k-row database; added to the V1 delivery gate). |
| AR-43 | Medium | No API versioning policy or compatibility promise for the public surface. | Resolved by spec §5.3 (public surface re-rooted under `/api/v1`; envelope and error registry documented as v1-stable, additive-only). |
| AR-44 | Low | No OpenAPI spec for external integrators. | Resolved by spec §8.10 (new `docs/api/openapi.yaml`, hand-maintained v1 public-surface spec). |
| AR-45 | Low | Missing runbooks — key rotation, super-admin recovery, restore drill, hot-DDL guidance. | Resolved by spec §7.4 (runbooks section added to 09, cross-referencing §2.4, §2.1, §7.1, §3.2). |
| AR-46 | Low | BR-API-1's "400" for the offset-cap violation doesn't exist in the closed error-code registry. | Documentation fix applied (spec §3.4 — offset above 10,000 returns `422 validation_failed`). |
| AR-47 | Low | 08's "only unauthenticated non-SPA endpoint" claim is false — public reads and `/api/auth/*` are also unauthenticated. | Documentation fix applied (spec §8.13 — 08's claim corrected to name `/readyz`, `/api/v1/auth/*`, and anonymous public reads). |

All Immediate/blocker items from §18 are resolved in the committed documentation set as of 2026-07-11.
