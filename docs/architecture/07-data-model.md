...
**Indexes.** PK on `id`; a partial index on `(status) WHERE deleted_at IS NULL` serving the public published-only scope (BR-API-2); a composite `(field, id)` B-tree per `indexed` field, serving the mandatory `id` tiebreaker sort directly (`02-core-interfaces.md` query.Builder invariant 7); and for every `unique` field a **partial unique index** `WHERE deleted_at IS NULL`.
