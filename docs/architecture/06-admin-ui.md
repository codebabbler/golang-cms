...
| Users & API keys | Role management within persona limits (P-2 cannot touch super admins); key creation shows the plaintext exactly once with a copy affordance (BR-AUTH-7). Admins can issue one-time password-reset tokens to users (BR-AUTH-13); the token is displayed exactly once. API keys include a `passwordReset` capability toggle for fine-grained access control. Includes an end-user tab: list, disable/enable, revoke sessions (BR-AUTH-14). |
