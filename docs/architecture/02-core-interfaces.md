...
httpapi ──► access.Evaluator ──► schema.Cache
   ...
   ├──► media.Service
   ├──► auth.{SessionService, APIKeyService, JWTService}
   └──► audit.Recorder
```
...
5. `Paginate` clamps limit to 100, rejects offset > 10,000, and always appends the `id` tiebreaker sort (BR-API-1).
...
The only write path into collection tables (BR-RBAC-5). Guarantees: unknown fields drop silently; `readOnlyForRoles` fields reject with field-level errors; type validation follows the field-type reference (numbers, datetimes, Tiptap JSONB shape for `richText`, finalized-media references for `media` — BR-MEDIA-3); `required` enforcement happens here, not in DDL (`03-dynamic-schema.md`).
...
| `auth.JWTService` | `Issue(endUser) (jwt, refresh)` · `Verify(jwt) (Claims, error)` · `Refresh(refreshToken) (jwt, refresh, error)` | RS256, 15-min TTL, identity-only claims (BR-AUTH-8); rotation + family revocation on reuse (BR-AUTH-9). |
